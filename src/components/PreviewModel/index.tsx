...
export const PreviewModel = ({handleClose=()=>false}:PreviewModelProps) => {

  const { setCurrentQuestionsFromPreview } = useQuestionStore();
  ...
  const handleTextAreaChange = (value: string) => {
    ...
    if (parsedQuestions.length > 0) {
      setPreviewData(parsedQuestions); // Clear the textarea after adding questions
    }
  };
  ...
  const parseQuestions = (input: string): Questions[] => {
    ...
    blocks.forEach((block, index) => {
      ...
      if (lines.length > 0) {
        ...
        if (question) {
            const options = lines.slice(1).map((option, idx) => ({
                id: idx + 1,
                option: option,
              }));
          questions.push({
  ...
  return (
    <div className="flex flex-row h-[37rem] rounded-lg border-2">
      {/* Left Section */}
