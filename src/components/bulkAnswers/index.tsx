...
export const BulkAnswers = () => {
  ...
  const handleSave = () => {
    ...
    const answersArray = bulkAnswersText.split('\n').filter((line) => line.trim() !== '')
  ...
  return (
    <div className='rounded-md'>
      <h2 className='text-2xl px-5 py-3 text-[#5A5A5A] font-semibold'>
        Add answers in bulk
      </h2>
      <div className='bg-[#F3F3F3] px-5 py-3'>
        <p className='text-md text-[#5A5A5A] font-bold mb-2'>
          Enter each answer choice on a separate line
        </p>
        <TextAreaComp
          className='h-[25.8rem] px-4 py-3 border-2'
